...
function renderList(artcList) {
  ...
  artcList.forEach(artc => {
    ...
    var $meta_date = $('span', true).addClass('created-date').text(helper.format(artc.created_at));
    var $meta_tag = $('span', true).addClass('tag').text(helper.map(artc.type) + '(' + artc.tags + ')');
